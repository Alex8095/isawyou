...
public class DictionaryServiceTest {
	...
	@Test
	@Transactional
	public void findDictionarys() {
		// List<Dictionary> dictionaries =
		// this.dictionaryService.findDictionaryAll();
		// assertEquals(0, dictionaries.size());
		// List<DictionaryValue> dictionaryValues =
		// this.dictionaryService.findDictionaryValueAll();
		// assertEquals(0, dictionaryValues.size());
		assertEquals(0, 0);
	}

	//
	// @Test
	// public void findSingledictionary() {
	// dictionary dictionary1 = this.dictionaryService.finddictionaryById(1);
	// assertTrue(dictionary1.getLastName().startsWith("Franklin"));
	// dictionary dictionary10 = this.dictionaryService.finddictionaryById(10);
	...
	public void insertDictionary() {
		...
		assertEquals("Verifying number of dictionarys after inserting a new one.", found + 1, dictionaries.size());
	}

	@Test
	@Transactional
	public void updateDictionary() throws Exception {
		Dictionary dictionary = this.dictionaryService.findDictionaryById(1);
		String old = dictionary.getName();
		dictionary.setName(old + "X");
		this.dictionaryService.saveDictionary(dictionary);
		dictionary = this.dictionaryService.findDictionaryById(1);
		assertEquals(old + "X", dictionary.getName());
	}

	// @Test
	// public void findPet() {
	// Collection<PetType> types = this.dictionaryService.findPetTypes();
	// Pet pet7 = this.dictionaryService.findPetById(7);
	// assertTrue(pet7.getName().startsWith("Samantha"));
	// assertEquals(EntityUtils.getById(types, PetType.class, 1).getId(),
	// pet7.getType().getId());
	// assertEquals("Jean", pet7.getdictionary().getFirstName());
	// Pet pet6 = this.dictionaryService.findPetById(6);
	// assertEquals("George", pet6.getName());
	// assertEquals(EntityUtils.getById(types, PetType.class, 4).getId(),
	// pet6.getType().getId());
	// assertEquals("Peter", pet6.getdictionary().getFirstName());
	// }
	//
	// @Test
	// public void getPetTypes() {
	// Collection<PetType> petTypes = this.dictionaryService.findPetTypes();
	//
	...
	@Test
	@Transactional
	public void insertDictionaryValue() {
		Dictionary dictionary6 = this.dictionaryService.findDictionaryById(3);
		int found = dictionary6.getDictionaryValues().size();
		DictionaryValue dictionaryValue = new DictionaryValue();
		dictionaryValue.setName("dictionaryValue");
		dictionary6.getDictionaryValues().add(dictionaryValue);
		assertEquals(found + 1, dictionary6.getDictionaryValues().size());
		// both storePet and storedictionary are necessary to cover all ORM tools
		this.dictionaryService.saveDictionaryValue(dictionaryValue);
		this.dictionaryService.saveDictionary(dictionary6);
		dictionary6 = this.dictionaryService.findDictionaryById(3);
		assertEquals(found + 1, dictionary6.getDictionaryValues().size());
	}
	@Test
	@Transactional
	public void updateDictionaryValue() throws Exception {
		DictionaryValue dictionaryValue3 = this.dictionaryService.findDictionaryValueById(3);
		String old = dictionaryValue3.getName();
		dictionaryValue3.setName(old + "X");
		...
		assertEquals(old + "X", dictionaryValue3.getName());
	}
	//
	// @Test
	// public void findVets() {
	// Collection<Vet> vets = this.dictionaryService.findVets();
	//
	...
	// this.dictionaryService.savePet(pet7);
	// pet7 = this.dictionaryService.findPetById(7);
	...
	//
	// System.out.println(dictionaryValue1.toString());
	//
	// DictionaryValue dictionaryValue = new DictionaryValue();
